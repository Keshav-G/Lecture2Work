...
def print_directory(db):
	other_data = ["Room 1", "Room 2", "Room 3", "Room 4", "Room 5"]
	for i, patient in enumerate(db):
		print("Name: {}, id: {}, age: {}".format(patient[0], patient[1],
								 patient[2]))
		print("Patient is in {}".format(other_data[i]))

def find_patient(id, db):
	for person in db:
		if person[1] == id:
			print("Found Results")
			return person[0] 

def add_test_result(id, db, test_name, test_result):
	for patient in db:
		if patient[1] == id:
